...
export class TypeBoolean {
  ...
  static getMongooseField (conf: ITextOrmField): any {
    const ret: any = {
      $type: Boolean,
      index: !!conf.index,
      default: conf.default ?? (conf.nullable ? null : false)
    }
  ...
  static async fromDb (data: any, conf: ITextOrmField): Promise<boolean | null> {
    if (conf.multiple) {
      data = ensureArray(data)
      data = data.filter((d: any) => conf.nullable || d !== null)
      return data.map((d: any) => !!d)
    }
    if (!data) return conf.default ?? (conf.nullable ? null : false)
    return !!data
  }

  static async toDb (data: any, conf: ITextOrmField): Promise<boolean | null> {
    if (conf.multiple) {
      data = ensureArray(data)
      data = data.filter((d: any) => conf.nullable || d !== null)
      return data.map((d: any) => !!d)
